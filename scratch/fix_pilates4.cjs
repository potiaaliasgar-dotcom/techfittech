...
function getSeriesScore(series, name) {
  ...
  // NX3 at the absolute top
  if (lowerName.includes('nx3')) {
    return -3000;
  }
  
  // All other bundles at the top
  if (lowerName.includes('bundle') || lowerName.includes('package')) {
    return -2000;
  }
...
if (plDataMatch) {
  ...
  plData.sort((a, b) => {
    ...
    // Secondary sort by name for all
