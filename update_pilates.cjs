...
html = html.replace(
  'const series=chip&&chip.textContent.trim()!==\'All\'?chip.textContent.trim():\'\';',
  'const oc = chip ? chip.getAttribute(\'onclick\') : \'\'; const mm = oc.match(/\\\'([^\\\']*)\\\'\\)$/); const series = mm ? mm[1] : \'\';'
);
